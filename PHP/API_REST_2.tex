	...
	\begin{frame}{Web API}
		\begin{center}
			\begin{tikzpicture}[node distance=1.5cm]
				...
				\node (api) [draw, rectangle, right of=client, xshift=2cm] {API Web};
				\node (db) [draw, cylinder, shape border rotate=90, aspect=0.3, below of=api, yshift=-1cm] {Database};
				
				\draw[->, thick] (client) -- node[above]{HTTP Request} (api);
				\draw[->, thick] (api) -- node[right]{Query} (db);
				\draw[->, thick] (db) -- node[left]{Dati} (api);
				\draw[->, thick] (api) -- node[below]{HTTP Response} (client);
			\end{tikzpicture}
		\end{center}
	\end{frame}
