...
\begin{frame}{La gestione del database}
	\begin{columns}
		...
		\begin{itemize}
			\item \textbf{Integrità sui dati}, in relazione ai valori che possono assumere e alle interdipendenze tra dati di differenti tabelle.
			\item \textbf{Integrità definite dall'utente}, cioè vincoli che sono specifici per un particolare database, come conseguenza di politiche commerciali dell'impresa oppure di norme legislative e fiscali.
		\end{itemize}
		...
		\begin{center}
			\begin{tikzpicture}[scale=0.7]
				% Tabella ORDINI
				\node[font=\scriptsize\bfseries, blue] at (0,4.2) {Tabella ORDINI};
				...
				\draw[fill=blue!10] (-2,3.3) rectangle (2,3);
				\node[font=\tiny] at (-1.3,3.15) {1};
				\node[font=\tiny] at (-0.3,3.15) {101};
				\node[font=\tiny] at (0.8,3.15) {500€};
				...
				\draw[fill=green!10] (-2,1) rectangle (2,0.7);
				\node[font=\tiny] at (-1.3,0.85) {101};
				\node[font=\tiny] at (-0.3,0.85) {Rossi};
				\node[font=\tiny] at (0.8,0.85) {1000€};
				...
				\node[font=\tiny, align=left] at (0,-0.6) {• Importo ≤ Credito};
				\node[font=\tiny, align=left] at (0,-0.85) {• ClienteID deve esistere};
				
				% Icona controllo
				\draw[fill=red!20, circle] (2.8,1.3) circle (0.4);
				\node[font=\Large, red!70!black] at (2.8,1.3) {✓};
...
\begin{frame}{La gestione del database}
	\begin{columns}
		...
		\begin{itemize}
			...
			\item Una \textbf{transazione} consiste in un insieme di operazioni di interrogazione o di modifica del database che devono essere eseguite come se fossero un'unica operazione.
		\end{itemize}
		
		\column{0.45\textwidth}
		\begin{center}
			\begin{tikzpicture}[scale=0.75]
				% Transazione esempio: Bonifico bancario
				\node[font=\scriptsize\bfseries, blue] at (0,4.2) {Esempio: Bonifico 100€};
				
				% BEGIN
				\draw[fill=green!20, rounded corners] (-2,3.8) rectangle (2,3.4);
				\node[font=\tiny\bfseries] at (0,3.6) {BEGIN TRANSACTION};
				
				% Operazione 1
				\draw[fill=blue!10, rounded corners] (-2,3.2) rectangle (2,2.7);
				\node[font=\tiny, align=center] at (0,2.95) {1. Conto A - 100€};
				
				% Operazione 2
				\draw[fill=blue!10, rounded corners] (-2,2.5) rectangle (2,2);
				\node[font=\tiny, align=center] at (0,2.25) {2. Conto B + 100€};
				
				% Decisione
				\draw[fill=yellow!20, diamond, aspect=2] (0,1.5) node[font=\tiny, align=center] {Tutte OK?} ;
				
				% Frecce
				\draw[->, thick, green!60!black] (-1.2,1.2) -- (-1.2,0.6);
				\node[font=\tiny, green!60!black, left] at (-1.2,0.9) {SÌ};
				
				\draw[->, thick, red] (1.2,1.2) -- (1.2,0.6);
				\node[font=\tiny, red, right] at (1.2,0.9) {NO};
				
				% COMMIT
				\draw[fill=green!30, rounded corners] (-2.2,0.6) rectangle (-0.2,0.1);
				\node[font=\tiny\bfseries, green!60!black] at (-1.2,0.35) {COMMIT};
				\node[font=\tiny, green!60!black] at (-1.2,0) {✓ Conferma};
				
				% ROLLBACK
				\draw[fill=red!30, rounded corners] (0.2,0.6) rectangle (2.2,0.1);
				\node[font=\tiny\bfseries, red!70!black] at (1.2,0.35) {ROLLBACK};
				\node[font=\tiny, red!70!black] at (1.2,0) {✗ Annulla tutto};
				
				% Principio atomicità
				\draw[fill=orange!15, rounded corners] (-2.2,-0.4) rectangle (2.2,-1);
				\node[font=\tiny, align=center, orange!80!black] at (0,-0.7) {\textbf{Atomicità:} Tutto o Niente};
...
\begin{frame}{La gestione del database}
	\begin{columns}
		\column{0.55\textwidth}
		\begin{itemize}
			\item Un esempio di transazione è un'operazione di \textbf{trasferimento di fondi} tra conti correnti bancari.
			\item L'importo trasferito deve essere tolto da un conto e aggiunto a un altro.
			\item Entrambe le operazioni devono essere eseguite per poter affermare di avere effettuato il trasferimento.
			\item Non è tollerabile che sia eseguita una sola delle due operazioni.
			\item Il supporto alle transazioni deve garantirne il corretto completamento anche in caso di \textbf{esecuzione concorrente}.
		\end{itemize}
		...
		\begin{center}
			\begin{tikzpicture}[scale=0.75]
				...
				% Stato finale errato (non ammesso)
				...
				\node[font=\tiny, align=center] at (0,-1.1) {\textbf{Gestione Concorrenza}\\Più transazioni simultanee};
