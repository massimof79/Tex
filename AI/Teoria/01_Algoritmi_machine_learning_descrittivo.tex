...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Apprendimento Non Supervisionato} (da solo): come quando organizzi i tuoi vestiti per colore senza che nessuno ti dica come fare. Il computer trova pattern da solo.
		
		\item \textbf{Apprendimento per Rinforzo} (per tentativi): come quando impari un videogioco provando e riprovando. Il computer impara dalle conseguenze delle sue azioni.
	\end{itemize}
	...
	Hai mai notato che di solito più studi, più bei voti prendi? O che più ti alleni, più veloce corri? La \textbf{regressione lineare} trova proprio queste relazioni!
	...
	\begin{esempio}
		Vogliamo capire se uno studente passerà l'esame in base alle ore di studio. Con 10 ore probabilmente sì, con 1 ora probabilmente no. Ma quanto è "probabile"? La regressione logistica ci dice la probabilità!
	\end{esempio}
	
	\subsection{La curva a S}
	
	Invece di una retta, usiamo una curva a forma di S chiamata \textbf{sigmoide}. Questa curva va sempre da 0 (0\% di probabilità) a 1 (100\% di probabilità).
	...
	Hai mai giocato a "20 domande"? Fai domande SÌ/NO per indovinare cosa pensa l'altro. Gli \textbf{alberi decisionali} funzionano esattamente così!
	...
	\begin{itemize}
		\item Vuoi capire PERCHÉ il computer ha fatto quella scelta
		\item Hai dati con categorie (tipo: piove/non piove, grande/piccolo)
		\item Vuoi spiegare la decisione ad altri
	\end{itemize}
	...
	\textbf{Vantaggi:}
	\begin{itemize}
		\item Facili da capire (anche tua nonna capirebbe!)
	...
	Immagina di avere caramelle rosse e blu sparse su un tavolo. Vuoi dividere il tavolo con un nastro in modo che da una parte ci siano solo le rosse e dall'altra solo le blu. La \textbf{SVM} trova il nastro "migliore"!
	...
	\begin{nota}
		Pensa a SVM come a un arbitro che vuole stare il più lontano possibile da entrambe le squadre per essere imparziale!
	\end{nota}
